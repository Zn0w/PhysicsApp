...
class GuiHandler:
    ...
    def createResultsWindow(self):
        ...
        self.x = Label(resultsWindow, text = "X: 0 at 0 seconds")
        self.x.pack()

        self.y = Label(resultsWindow, text = "Y: 0 at 0 seconds")
        self.y.pack()
    ...
    def getXandY(self):
        ...
        if float(time) > self.mathHandler.flyTime:
            self.x.config(text = "X: " + str(self.mathHandler.getXPosition(self.mathHandler.flyTime)) + " at " + time + " seconds")
            self.y.config(text = "Y: " + str(self.mathHandler.getYPosition(self.mathHandler.flyTime)) + " at " + time + " seconds")
        else:
            self.x.config(text = "X: " + str(self.mathHandler.getXPosition(float(time))) + " at " + time + " seconds")
            self.y.config(text = "Y: " + str(self.mathHandler.getYPosition(float(time))) + " at " + time + " seconds")